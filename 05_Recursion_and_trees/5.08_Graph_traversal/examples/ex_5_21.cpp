...
template <typename F>
void depth_first_search(vector<link<int>>& adv, int k, vector<bool>& visited, F visit) {
    visit(k);
    ...
    for(link<int> t = adv[k]; t; t = t->next) {
        if(!visited[t->item])
            depth_first_search(adv, t->item, visited, visit);
    }
}
...
int main() {
    ...
    depth_first_search(adv, 0, visited, [](int k){
        cout << k;
        cout << endl;
    });
